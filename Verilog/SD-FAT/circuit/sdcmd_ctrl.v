...
//--------------------------------------------------------------------------------------------------------
// ND-120 project modification (11-JUL-2026, Ronny Hansen): the sdcmd inout
// is split into explicit sdcmd_i / sdcmd_o / sdcmd_oe ports so the single
// tristate driver lives at the board top level only (repo rule: no 'z'
// inside the FPGA) and the CMD line can be shared with the SD writer.
//--------------------------------------------------------------------------------------------------------

module sdcmd_ctrl (
    input  wire         rstn,
    input  wire         clk,
    // SDcard signals (sdclk and sdcmd; tristate resolved at the top level)
    output reg          sdclk,
    input  wire         sdcmd_i,
    output wire         sdcmd_o,
...
// sdcmd driver (tristate resolved at the top level; ND-120 mod)
