...
module nd_storage_ticks_tb;
  ...
  // TICK INSTRUMENTATION (copy of nd_storage_tb.v): clk_cpu ticks from the
  // request edge until busy falls, i.e. the whole cost of ONE 2048-byte
  // (1024-word) block read through nd_storage.
  integer nds_tick = 0;
  always @(posedge clk_cpu) nds_tick = nds_tick + 1;
  integer rd_t0;
  ...
  task do_read(input [2:0] c, input [15:0] blk, input [127:0] what);
    begin
      ...
      $display("[TICKS] client %0d block %0d (%0s): %0d clk_cpu ticks",
               c, blk, what, nds_tick - rd_t0 - 20);
    end
  endtask
