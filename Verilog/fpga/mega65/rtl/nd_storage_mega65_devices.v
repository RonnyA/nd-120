...
/****************************************************************************
** nd_storage_mega65_devices - the ND-120 storage subsystem on the MEGA65  **
**                                                                         **
** Full path: Verilog/fpga/mega65/rtl/nd_storage_mega65_devices.v          **
**                                                                         **
** Copied 02-SEP-2026 from fpga/mister/rtl/nd_storage_mister_devices.v -   **
** the "storage subsystem in a box" with pin-for-pin FDISK_* and FDBUF_*,  **
** WDISK_* and WDBUF_* and TAPE byte-source seams matching ND120_CORE, two **
** floppy drives, two Winchester units and the paper tape on the same      **
** three controller-facing adapters. Read that file's header for the slot  **
** map, the two-adapters-per-controller OR, the open semantics and the     **
** floppy media-format rule; all of it is unchanged. The ONLY difference:   **
** the backend is nd_storage_vdrives (MiSTer2MEGA65's virtual drives,      **
** byte-wide buffer bus in the QNICE clock domain) instead of              **
** nd_storage_vdrives, so the sd_buff_* ports here are 8 bits wide with a      **
** 14-bit byte address, and img_size is 32 bits.                            **
**                                                                         **
** SLOT MAP (= the mount lines in CORE/vhdl/config.vhd, in order, =        **
** vdrives VDNUM index, = nd_storage_vdrives client):                       **
**   0  floppy drive 0    1  floppy drive 1                                 **
**   2  Winchester unit 0 3  Winchester unit 1                              **
**   4  paper tape                                                          **
**                                                                         **
** Ronny Hansen, 02-SEP-2026                                                **
*****************************************************************************/

module nd_storage_mega65_devices #(
    parameter integer N_CLIENTS = 5     // fixed by the slot map; kept as a parameter for the machine top
) (
    input  wire clk_cpu,
    ...
    // ---- vdrives block interface (clk_sys = QNICE clock), flattened per slot ----
  ...
  // one open pulse after reset, for every adapter at once (nd_storage_vdrives
  ...
  // ---- the backend -------------------------------------------------------------
  nd_storage_vdrives #(.N_CLIENTS(N)) u_vd (
