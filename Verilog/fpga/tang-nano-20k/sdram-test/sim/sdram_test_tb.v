/****************************************************************************
** Testbench for the Tang Nano 20K SDRAM test                              **
**                                                                         **
** Runs the full test top against the behavioral SDRAM model, decodes the  **
** UART output and echoes it to the console. The UART start trigger is     **
** exercised by sending a character (like typing a key in a terminal).     **
** Fast parameters: 16 clocks/bit UART, 64-byte block test.                **
**                                                                         **
...
module sdram_test_tb;
  ...
  localparam UART_DIV = 16;  // clocks per bit in this sim (BAUD = 27M/16)
  localparam BITP = UART_DIV * CLK_PERIOD;

  reg clk27 = 0;
  always #(CLK_PERIOD / 2.0) clk27 = ~clk27;
