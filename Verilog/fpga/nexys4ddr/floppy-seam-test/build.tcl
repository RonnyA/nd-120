# Floppy seam probe for the Nexys 4 DDR (floppy_seam_top) - same in-memory
...
# Purpose and output format: see the header of floppy_seam_top.v and
...
read_verilog [list \
    [file join $srcdir floppy_seam_top.v] \
...
read_xdc [file join $srcdir floppy_seam.xdc]

# nd_storage `include "sd_fat_features.vh" / "nd_storage_status.vh"
synth_design -top floppy_seam_top -part $part \
...
write_bitstream -force [file join $srcdir floppy_seam.bit]

if {$noburn} {
    puts "=== FLOPPY SEAM BUILD COMPLETE (not programmed) ==="
    exit 0
}
...
set_property PROGRAM.FILE [file join $srcdir floppy_seam.bit] $dev
program_hw_devices $dev
puts "PROGRAMMED (JTAG)"
close_hw_manager
puts "=== FLOPPY SEAM BUILD COMPLETE ==="
