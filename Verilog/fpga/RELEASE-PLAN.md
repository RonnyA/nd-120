...
| `nd120_mega65_rev3_<clk>_115200.cor` | MEGA65 rev 3 | `.cor` | TBD | NEW - work in progress (separate effort). Rev-3-specific core. |
| `nd120_mega65_r6_<clk>_115200.cor` | MEGA65 newer revs | `.cor` | TBD | NEW - work in progress. Core for the newer board revisions. |
...
- MEGA65 ships TWO cores because the hardware revisions differ (rev 3 vs the
  newer revisions); each is a separate build. That work is in progress in a
  parallel effort - do not stage a MEGA65 core until that effort confirms it.
