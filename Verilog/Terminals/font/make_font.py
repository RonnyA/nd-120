...
def write_font_rom_v(rom, path, src):
    """Emit font_rom.v with the glyph data EMBEDDED - no $readmemh, no file.

    WHY EMBEDDED. font_rom.v used to $readmemh("font8x16.hex"), a loose data
    file. Every FPGA tool resolves that relative path differently (Vivado
    against its working dir, Quartus via SEARCH_PATH, Verilator/iverilog
    against the sim dir), so each board kept its OWN copy of the hex - and
    copies drift. A stale copy in the Nexys working dir was read for two days
    while every font edit sat unused (01/02-SEP-2026); MiSTer had the same
    class of bug with its own stale copy. Embedding the data in the .v removes
    the file entirely: every board already lists font_rom.v, so it is compiled
    identically everywhere with nothing to find, copy or let go stale. This
    matches term_banner_rom.v and term_panel_rom.v, which are generated the
    same way.

    The reg-array + registered read is the canonical block-RAM template - it
    infers a block RAM on Xilinx and Intel exactly as the $readmemh form did,
    just with the init values inline. rom is 512 glyphs x 16 bytes = 8192.
    """
    bytes_flat = [b for glyph in rom for b in glyph]
    assert len(bytes_flat) == 8192, len(bytes_flat)
    lines = []
    lines.append("//============================================================================\n")
    lines.append("//! 8x16 character generator ROM - GENERATED by font/make_font.py.\n")
    lines.append("//! DO NOT HAND-EDIT: re-run  python3 font/make_font.py <psf> font8x16.hex\n")
    lines.append("//! (from Verilog/Terminals/) and commit both the .hex and this .v.\n")
    lines.append("//!\n")
    lines.append("//! source font: %s\n" % src)
    lines.append("//!\n")
    lines.append("//! Four 128-glyph pages x 16 rows x 8 pixels = 8192 bytes. Address is\n")
    lines.append("//! {char_code, row}; char_code is {page[1:0], code[6:0]}:\n")
    lines.append("//!   page 0 = US / ISO 646 IRV      page 1 = Norwegian (NS 4551-1)\n")
    lines.append("//!   page 2 = TDV2200 char set 2    page 3 = DEC Special Graphics\n")
    lines.append("//! page 2 is the TDV build's box set (ESC 6 and SS2); page 3 is the\n")
    lines.append("//! VT100 build's line drawing. GFX_PAGE in text_screen.v picks which.\n")
    lines.append("//!\n")
    lines.append("//! The DATA IS EMBEDDED below (no $readmemh, no font8x16.hex) so this ROM\n")
    lines.append("//! compiles identically on every board and tool with nothing to find or\n")
    lines.append("//! let go stale - see write_font_rom_v() in make_font.py for the full why.\n")
    lines.append("//! One clock of read latency, registered output - infers a block RAM on\n")
    lines.append("//! both Xilinx and Intel rather than a pile of LUTs.\n")
    lines.append("//============================================================================\n")
    lines.append("\n")
    lines.append("`default_nettype none\n")
    lines.append("\n")
    lines.append("module font_rom #(\n")
    lines.append("    //! Ignored - the font data is embedded below, not read from a file.\n")
    lines.append("    //! Kept so existing instantiations that pass .FONT_FILE(...) still\n")
    lines.append("    //! elaborate unchanged.\n")
    lines.append("    parameter FONT_FILE = \"embedded\"\n")
    lines.append(") (\n")
    lines.append("    input  wire       clk,        //! pixel clock\n")
    lines.append("    input  wire [8:0] char_code,  //! {page[1:0], code[6:0]}\n")
    lines.append("    input  wire [3:0] row,        //! which of its 16 pixel rows\n")
    lines.append("    output reg  [7:0] pixels      //! that row, MSB = leftmost. Valid one clock later\n")
    lines.append(");\n")
    lines.append("\n")
    lines.append("  (* rom_style = \"block\" *)\n")
    lines.append("  reg [7:0] s_rom[0:8191];\n")
    lines.append("\n")
    lines.append("  initial begin\n")
    for addr, b in enumerate(bytes_flat):
        lines.append("    s_rom[13'd%d] = 8'h%02X;\n" % (addr, b))
    lines.append("  end\n")
    lines.append("\n")
    lines.append("  always @(posedge clk) begin\n")
    lines.append("    pixels <= s_rom[{char_code, row}];\n")
    lines.append("  end\n")
    lines.append("\n")
...
def main():
    ...
    # ALSO emit the ROM as embedded Verilog, next to font_rom.v. This is the
    # real deliverable for synthesis - the .hex above is kept only as the
    # human-readable master and the testbench model source. See
    # write_font_rom_v() for why the font is embedded rather than $readmemh'd.
    rom_v = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "..", "rtl", "font_rom.v")
    write_font_rom_v(rom, os.path.normpath(rom_v), src)
    ...
    print("wrote %s: embedded ROM (%d bytes)" % (os.path.normpath(rom_v),
                                                 sum(len(g) for g in rom)))
    if gfx_absent:
