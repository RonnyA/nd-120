/****************************************************************************
** MUX21L - exhaustive functional testbench                               **
**                                                                         **
** WHAT THIS MODULE ACTUALLY IS                                            **
**   MUX21L wraps a single Multiplexer_2 (muxIn_0=A, muxIn_1=B, sel=S) and **
**   inverts the multiplexer's output through the s_zn_out=~s_z assign:    **
**       ZN = ~( S ? B : A )                                               **
**                                                                         **
** SURPRISE FOUND: MUX21LP.v, right next to this file, instantiates the    **
**   SAME Multiplexer_2 with the SAME port mapping (muxIn_0=A, muxIn_1=B,  **
**   sel=S) and inverts it the same way (ZN = ~wire_plexer_out). The two   **
**   modules are FUNCTIONALLY IDENTICAL - same truth table, only the local **
**   wire names differ (s_a/s_b/s_s/s_z here, direct port refs there).     **
**   This testbench and MUX21LP_tb.v run the identical reference model     **
**   against both DUTs and both pass, confirming the duplication.          **
**                                                                         **
...
**                                                                         **
** Run: cd Verilog/Shared/ndlib/sim && iverilog -g2012 -o MUX21L_tb.vvp \
**          MUX21L_tb.v -y .. -y ../../logisim && vvp MUX21L_tb.vvp        **
**                                                                         **
...
module MUX21L_tb;
  ...
  initial begin
    $dumpfile("MUX21L_tb.vcd");
    $dumpvars(0, MUX21L_tb);
    ...
    $display("=====================================================");
    $display(" MUX21L exhaustive functional testbench");
