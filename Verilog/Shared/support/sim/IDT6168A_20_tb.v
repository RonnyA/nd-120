...
module IDT6168A_20_tb;
  ...
  localparam integer CHECKS_EXPECTED = 10;
  ...
  initial begin
    ...
    // ---------------------------------------------------------------
    // Summary (fixed expected check count: a silent partial run fails)
    // ---------------------------------------------------------------
    $display("");
    $display("================================================================");
    $display(" IDT6168A_20 testbench summary: %0d passed / %0d failed", pass_count, fail_count);
    $display("================================================================");
    if (fail_count == 0 && pass_count == CHECKS_EXPECTED)
      $display("TB_RESULT: PASS (%0d checks)", pass_count);
    else
      $display("TB_RESULT: FAIL (%0d failed / %0d passed, expected %0d checks)",
               fail_count, pass_count, CHECKS_EXPECTED);
