...
1. `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/DELILAH-CPU/CGA_INTR/circuit/CGA_INTR.v`
...
2. `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/DELILAH-CPU/CGA_INTR/circuit/CGA_INTR_IRSRC.v`
...
3. `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/CPU-BOARD-3202/circuit/CPU_PROC_CGA_33.v`
   - Line 37 comment fix only: `//! Memory Operation Ready` → `//! Memory Out of Range` (the port was mislabeled; it is MOR).

4. `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/CPU-BOARD-3202/circuit/MEM_43.v`
...
**NOT mine** (also uncommitted, belongs to the status-fence workstream — do not attribute to this handoff): `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/DELILAH-CPU/CGA_INTR/circuit/CGA_INTR_CNTLR_VECGEN_STAT.v` and `.../CGA_INTR_CNTLR_VECGEN_STAT_SBIT.v`.
...
- **Timeout generator** `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/DECODE-GateArray/DGA/circuit/DECODE_DGA_POW.v`:
...
- **Split into MOR vs IOXERR** `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/CPU-BOARD-3202/circuit/BIF_BCTL_BDRV_7.v`:
...
- **Level-12 gate** `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/DELILAH-CPU/CGA_INTR/circuit/CGA_INTR_IRSRC.v`: `GATES_5` NAND(`FIDBO[12]`,EMPID) + `GATES_21` NOR(...,`s_mor_n`) → `IREQ_15_0_N[12]`. Asserts on either a software `FIDBO[12]` write or MOR active.
...
- `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/PAL/PAL_44445B.v:85` (CPU side, `CLRQ_n`) and `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/PAL/PAL_44446B.v:85` (bus side, `AOK = ~(BMEM_n|BD23|BD22|BD21|MOFF)`): onboard only when `PPN[23:21]=000` / `BD[23:21]=000` = bottom 2M words = 4 MB.
...
The Verilator build does **not** use the SIP1M9 chip model. `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/CPU-BOARD-3202/circuit/MEM_43.v` selects, for `VERILATOR_SIM`, `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/CPU-BOARD-3202/circuit/MEM_RAM_49_SIM.v` — a flat behavioral model, 3 banks × 1M × 18 bits = **6 MB**, onboard 4 MB window fully backed. So MOR is testable in sim with a clean boundary (< 4 MB never fires; ≥ 4 MB, no responder, fires). The C++ harnesses preload via its `b0_*` arrays (e.g. `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/runSim/Run120.cpp:498`). **No RAM change is needed.** (SIP1M9 path `MEM_RAM_49.v` is the historical `else` fallback, compiled by no current build.)

Bus-ack path: `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/ND120_TOP.v:180 BDRY_n_IN = 1'b1` and `:714 s_bus_bdry_in_n = BDRY_n_IN & s_dev_bdry_n` — only the device model (`simDevices/NDBus.cpp` / `NDDevices.cpp`) can ack a bus cycle. An address no device claims leaves BDRY deasserted → timeout → MOR. (Not runtime-confirmed that NDBus withholds BDRY for unmapped addresses — see open questions.)

## The real RUN blocker (do NOT expect the MOR wiring to fix this)

Per `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/docs/RUN-level14-livelock-analysis.md:120-139`: with the status fence now working, RUN reaches the level-14 handler and INSTRUCTION-B aborts with `Internal Interrupt. IIC: 11 - Memory Out of Range`. But the measured hardware vector is 2 = IREQ bit 10 = **IOX error** (the reference ND-110 prints "IOX-ERROR STARTED"). So the **vector→IIC decode path is wrong** — an IOX error is being reported as MOR. Suspects named in that doc: the PICV/PICS→IDB path (`CGA_IDBCTL` SEL6 muxes, IDBS, PICVC) and the AIIC / `TRA IIC` microcode expectations (csa 00725–00731). Probe suggested: capture the IDB value the microcode reads at AIIC together with `PICV_2_0`/`PICS_2_0` and compare to the IIC code INSTRUCTION-B expects for IOX. This is the actual RUN work and is separate from the MOR tie-off.

## Verification status (be honest with Ronny)

- **Compiles:** yes — `make -C runSim compile USE_LATCHES=0` → exit 0 with the wiring in.
- **Unit test:** `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/DELILAH-CPU/CGA_INTR/sim/test_intr.cpp` prints "Passed" but its output checks are compiled out behind `#ifdef ___later___` — it drives MORN but asserts nothing. **Not real verification.** (Also not registered in `/mnt/e/Dev/Repos/Ronny/nd-120/Verilog/tests/run_all_tests.sh`.)
